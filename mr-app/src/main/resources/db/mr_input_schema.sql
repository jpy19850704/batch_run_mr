...
-- SA-CCR 交易范围与交易对手/净额集合归属表
CREATE TABLE IF NOT EXISTS SACCR_TRADE_CP (
    DATA_DATE           DATE     NOT NULL COMMENT '数据日期，yyyy-MM-dd',
    INSTRUMENT_ID       VARCHAR(128)    NOT NULL COMMENT '交易唯一标识',
    COUNTERPARTY_ID     VARCHAR(128)    NOT NULL COMMENT '交易对手ID',
    NETTING_SET_ID      VARCHAR(128)             COMMENT '净额集合ID；为空表示单笔独立净额模式',
    CREATED_AT          TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
    UPDATED_AT          TIMESTAMP       DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (DATA_DATE, INSTRUMENT_ID)
) COMMENT='SA-CCR交易范围与归属';

-- SA-CCR 净额集合保证金协议表
CREATE TABLE IF NOT EXISTS SACCR_NETTING_SET (
    DATA_DATE               DATE     NOT NULL COMMENT '数据日期，yyyy-MM-dd',
    NETTING_SET_ID          VARCHAR(128)    NOT NULL COMMENT '净额集合ID',
    COUNTERPARTY_ID         VARCHAR(128)    NOT NULL COMMENT '交易对手ID',
    MARGIN_TYPE             VARCHAR(32)     NOT NULL COMMENT 'NONE/BILATERAL/ONE_WAY_BANK',
    MARGIN_CCY              VARCHAR(8)               COMMENT 'TH/MTA/NICA统一币种',
    MARGIN_FX_RATE_TO_CNY   DECIMAL(38,10)           COMMENT '保证金币种兑人民币汇率',
    THRESHOLD               DECIMAL(38,10)           COMMENT '保证金币种下TH',
    MTA                     DECIMAL(38,10)           COMMENT '保证金币种下MTA',
    NICA                    DECIMAL(38,10)           COMMENT '保证金币种下NICA',
    MPOR_DAYS               INT                      COMMENT '显式MPOR工作日',
    CREATED_AT              TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
    UPDATED_AT              TIMESTAMP       DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (DATA_DATE, NETTING_SET_ID)
) COMMENT='SA-CCR净额集合保证金协议';

-- SA-CCR 押品明细表
CREATE TABLE IF NOT EXISTS SACCR_COLLATERAL_DETAIL (
    DATA_DATE           DATE     NOT NULL COMMENT '数据日期，yyyy-MM-dd',
    COLLATERAL_ID       VARCHAR(128)    NOT NULL COMMENT '押品唯一标识',
    COLLATERAL_SCOPE    VARCHAR(32)     NOT NULL COMMENT 'NETTING_SET/TRADE',
    NETTING_SET_ID      VARCHAR(128)             COMMENT '净额集合ID',
    INSTRUMENT_ID       VARCHAR(128)             COMMENT '交易唯一标识',
    COLLATERAL_TYPE     VARCHAR(32)     NOT NULL COMMENT 'CASH/BOND/EQUITY/FUND/GOLD/OTHER',
    DIRECTION           VARCHAR(16)     NOT NULL COMMENT 'RECEIVED/POSTED',
    COLLATERAL_CCY      VARCHAR(8)      NOT NULL COMMENT '押品币种',
    MARKET_VALUE        DECIMAL(38,10)  NOT NULL COMMENT '押品市值',
    FX_RATE_TO_CNY      DECIMAL(38,10)  NOT NULL COMMENT '押品币种兑人民币汇率',
    HAIRCUT_RATE        DECIMAL(18,10)  NOT NULL COMMENT '折扣率',
    ELIGIBLE_FLAG       VARCHAR(8)      NOT NULL COMMENT '是否合格押品：1/0、Y/N、TRUE/FALSE',
    CREATED_AT          TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
    UPDATED_AT          TIMESTAMP       DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (DATA_DATE, COLLATERAL_ID)
) COMMENT='SA-CCR押品明细';
...
CREATE TABLE IF NOT EXISTS CVA_COUNTERPARTY (
    DATA_DATE        DATE         NOT NULL COMMENT '数据日期，yyyy-MM-dd',
    COUNTERPARTY_ID  VARCHAR(128) NOT NULL COMMENT '交易对手ID',
    INDUSTRY         VARCHAR(32)  NOT NULL COMMENT '交易对手行业分类',
    CREDIT_QUALITY   VARCHAR(16)  NOT NULL COMMENT 'IG/HY_NR',
    CREATED_AT       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UPDATED_AT       TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS CVA_NETTING_SET (
    ...
    NETTING_SET_ID             VARCHAR(128) NOT NULL COMMENT '净额集合ID',
    COUNTERPARTY_ID            VARCHAR(128) NOT NULL COMMENT '交易对手ID',
    EFFECTIVE_MATURITY_YEARS   DECIMAL(38,10) NOT NULL COMMENT 'M_NS，有效期限，年',
    CVA_SCOPE_FLAG              VARCHAR(16)  NOT NULL COMMENT 'IN_SCOPE/OUT_OF_SCOPE',
    CREATED_AT                  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UPDATED_AT                  TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (DATA_DATE, NETTING_SET_ID)
...
CREATE TABLE IF NOT EXISTS CVA_HEDGE (
    DATA_DATE                    DATE         NOT NULL COMMENT '数据日期，yyyy-MM-dd',
    HEDGE_ID                     VARCHAR(128) NOT NULL COMMENT '对冲交易ID',
    HEDGE_TYPE                   VARCHAR(32)  NOT NULL COMMENT 'SINGLE_NAME_CDS/SINGLE_NAME_CONTINGENT_CDS/INDEX_CDS',
    COUNTERPARTY_ID              VARCHAR(128) COMMENT '单名对冲对应交易对手，指数对冲为空',
    RELATION_TYPE                VARCHAR(32) COMMENT 'DIRECT/LEGAL_RELATION/SAME_SECTOR_REGION',
    REFERENCE_INDUSTRY           VARCHAR(32) COMMENT '对冲参考主体行业',
    REFERENCE_CREDIT_QUALITY     VARCHAR(16) COMMENT '对冲参考主体信用水平：IG/HY_NR',
    INDEX_BASE_RISK_WEIGHT       DECIMAL(18,10) COMMENT '指数成分加权基础风险权重',
    INDEX_DIVERSIFIED_FLAG      VARCHAR(1)   COMMENT '指数是否按0.7进行分散化调整：Y/N',
    REMAINING_MATURITY_YEARS     DECIMAL(38,10) NOT NULL COMMENT '对冲剩余期限，年',
    NOTIONAL_CNY                 DECIMAL(38,10) NOT NULL COMMENT '对冲名义本金，人民币',
    CREATED_AT                   TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UPDATED_AT                   TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
