...
public class ImaEsResultDetailPersistService {
    ...
    private static final String TARGET_TABLE = "TB_OUT_IMA_ES_RESULT_DETAIL";
    ...
    public void deleteByBatch(String batchId) {
        if (batchId == null || batchId.trim().isEmpty()) {
            throw new IllegalArgumentException("IMA ES 明细结果清理缺少 BATCH_ID");
        }
        int deleted = jdbcTemplate.update("DELETE FROM " + TARGET_TABLE + " WHERE BATCH_ID=?", batchId);
        log.info("清理 IMA ES 明细历史结果: batchId={}, deleted={}", batchId, deleted);
    }

    @Transactional(transactionManager = "engineResultDbTransactionManager", rollbackFor = Exception.class)
    public void persist(List<ImaEsResultDetail> details) {
        if (details == null || details.isEmpty()) {
            log.warn("IMA ES 明细结果为空，跳过落库");
            return;
        }
        ...
        log.info("IMA ES 明细结果落库完成: batchId={}, rows={}", batchId, details.size());
    }
    ...
    private static String requireBatchId(ImaEsResultDetail detail) {
        if (detail == null || detail.getBatchId() == null || detail.getBatchId().trim().isEmpty()) {
            throw new IllegalArgumentException("IMA ES 明细结果缺少 BATCH_ID");
        }
