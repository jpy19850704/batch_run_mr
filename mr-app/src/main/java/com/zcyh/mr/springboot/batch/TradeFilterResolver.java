...
import com.zcyh.mr.springboot.batch.model.BatchTradeFilter;
...
public class TradeFilterResolver {
    ...
    @Autowired
    public TradeFilterResolver(RuleDefinitionRepository ruleDefinitionRepository) {
        this.ruleDefinitionRepository = ruleDefinitionRepository;
    }

    public InputFilterExpression resolve(BatchTradeFilter filter) {
        if (filter == null) {
            return null;
        }
    ...
    private InputFilterExpression loadRuleFilterTree(String ruleId) {
        String ruleLabel = "TRADE 交易过滤规则";
        RuleDefinitionRow row = ruleDefinitionRepository.findRequired(RULE_TYPE_TRADE, ruleId, ruleLabel);
        JSONObject json = JSON.parseObject(row.getRuleJson());
        Object filterTreeValue = json == null ? null : json.get("filterTree");
        if (filterTreeValue == null) {
            throw new IllegalArgumentException(ruleLabel + "缺少 filterTree: " + ruleId);
        }
        InputFilterExpression filterTree = JSON.parseObject(
    ...
    private static void validateFilterExpression(InputFilterExpression node,
                                                 String path,
                                                 int depth,
                                                 int[] nodeCount) {
        if (node == null) {
    ...
    private static void validateFilterValue(InputFilterExpression node, String path) {
