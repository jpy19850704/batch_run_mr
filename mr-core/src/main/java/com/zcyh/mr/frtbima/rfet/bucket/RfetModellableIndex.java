 ...
 *   boolean ok = index.isModellable("CNY_SWAP", 365);
 *   // 二维（波动率曲面）：(tenor, delta) 是否落在某可建模桶的范围内
 *   boolean ok = index.isModellable("CSI300_VOL", 180, 0.25);
 *   // 曲线级：该 curveId 是否存在任意可建模桶（用于 FX/Vol 整曲线替换）
 *   boolean ok = index.hasAnyModellableBucket("USDCNY_FWD");
 * </pre>
 */
public class RfetModellableIndex {

    /** curveId → 该曲线所有可建模桶的结果列表 */
    ...
    public static RfetModellableIndex build(List<RfetResult> results) {
        ...
        if (results != null) {
            for (RfetResult r : results) {
                if (r.isModellable()) {
                    map.computeIfAbsent(r.getCurveId(), k -> new ArrayList<>()).add(r);
                }
            }
        }
     ...
     */
    public boolean isModellable(String curveId, int tenorDays, boolean reducedSetOnly) {
        List<RfetResult> buckets = index.get(curveId);
    ...
    /** 等价于 isModellable(curveId, tenorDays, false)，默认 Full Set 匹配 */
    public boolean isModellable(String curveId, int tenorDays) {
        return isModellable(curveId, tenorDays, false);
    }
     ...
     * @param delta          ITM 概率（Row D），null 时退化为一维匹配
     * @param reducedSetOnly true=仅匹配 Reduced Set 桶
     * @return true 表示该点可建模
     */
    public boolean isModellable(String curveId, int tenorDays, Double delta, boolean reducedSetOnly) {
        if (delta == null) {
            return isModellable(curveId, tenorDays, reducedSetOnly);
        }
        List<RfetResult> buckets = index.get(curveId);
        if (buckets == null) return false;
        for (RfetResult r : buckets) {
    ...
    /** 等价于 isModellable(curveId, tenorDays, delta, false)，默认 Full Set 匹配 */
    public boolean isModellable(String curveId, int tenorDays, Double delta) {
        return isModellable(curveId, tenorDays, delta, false);
    }
     ...
     */
    public boolean hasAnyModellableBucket(String curveId, boolean reducedSetOnly) {
        List<RfetResult> buckets = index.get(curveId);
        if (buckets == null) return false;
    ...
    /** 等价于 hasAnyModellableBucket(curveId, false)，默认 Full Set */
    public boolean hasAnyModellableBucket(String curveId) {
        return hasAnyModellableBucket(curveId, false);
    }

    /**
     * 返回指定 curveId 的所有可建模桶结果（只读）。
     */
    public List<RfetResult> getBuckets(String curveId) {
        List<RfetResult> buckets = index.get(curveId);
