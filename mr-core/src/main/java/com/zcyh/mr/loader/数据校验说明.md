...
系统在 `Loader` 解析阶段对输入数据进行全面校验，校验不通过的数据自动跳过/剔除，不影响正常数据的计算。所有校验错误汇总到 `log_data` 数组输出。
...
由 `TradeValidator` 根据 [validationRules.json](file:///d:/后端代码/engine/src/main/java/com/zcyh/mr/loader/validationRules.json) 配置文件逐笔校验，**校验失败的交易整笔跳过**。
...
| 文件 | 职责 |
|------|------|
| [validationRules.json](file:///d:/后端代码/engine/src/main/java/com/zcyh/mr/loader/validationRules.json) | 校验规则配置 |
...
| [Calc.java](file:///d:/后端代码/engine/src/main/java/com/zcyh/mr/calc/Calc.java) | 校验错误合并到 log_data |
