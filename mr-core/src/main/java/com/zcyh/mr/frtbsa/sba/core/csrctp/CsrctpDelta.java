...
/**
 * CSR CTP Delta 资本计算
 * 信用利差风险 — 证券化（关联交易组合）Delta 敏感性
 *
 * 根据 MAR21.56-21.64：
 * - 桶内相关性：同一发行人 ρ = rhoSameName (0.90)，不同发行人 ρ = rhoBase (0.42)
 * - 跨桶相关性：γ = 0.18
 * - 三场景（M/H/L）同时计算，取 max
 */
public class CsrctpDelta {

    private static final String SENS_TYPE = FrtbConstants.SENS_DELTA;
    private static final String RISK_CLASS = FrtbConstants.RISK_CLASS_CSRCTP;
    private static final HashMap<String, Double> CSRCTP_DELTA_GAMMA_MATRIX = FrtbParamsCache.buildCtpDeltaGammaMatrix();
     ...
     * 桶内 KL 矩阵计算
     * CTP 相关性规则（MAR21.62）：
     * - 同一发行人(riskFactorId 相同)：ρ = 1.0
     * - 同桶不同发行人：ρ = rhoBase (0.42)
     */
    private List<Map<String, Object>> calculateKL(List<Map<String, Object>> dataList) {
        ...
        double baseRho = FrtbParamsCache.getCtpRhoBase();
        ...
        for (Map<String, Object> k : dataList) {
            ...
            for (Map<String, Object> l : dataList1) {
                ...
                double tenorL = parseTenor(l.get("riskFactorVertex1"));
                boolean sameIssuer = k.get("riskFactorId").equals(l.get("riskFactorId"));
                boolean sameType = riskTypeK.equals(riskTypeL);
                boolean isSelf = sameIssuer && isSameVertex(tenorK, tenorL) && sameType;
                double currentBaseRho;
                if (isSelf) {
                    currentBaseRho = 1.0;
                } else {
                    double rhoName = sameIssuer ? FrtbParamsCache.getCtpRhoSameName() : baseRho;
                    double rhoBasis = sameType ? 1.0 : FrtbParamsCache.getCtpRhoBasisDiff();
                    currentBaseRho = rhoName * rhoBasis;
                }
    ...
    private double lookupGamma(String bucketB, String bucketC) {
        ...
        Double gamma = CSRCTP_DELTA_GAMMA_MATRIX.get(bucketB + "," + bucketC);
        if (gamma == null) {
            throw new IllegalArgumentException("未配置 CSRCTP Delta 跨桶相关性: " + bucketB + "," + bucketC);
        }
        return gamma;
    }
