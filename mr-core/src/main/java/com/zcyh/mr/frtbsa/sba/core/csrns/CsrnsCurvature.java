...
/**
 * CSRNS Curvature 资本计算逻辑
 * 非证券化信用利差风险 - Curvature敏感性
 */
public class CsrnsCurvature {
    ...
    public Map<String, Object> calculate(List<Map<String, Object>> dataList, Boolean needDecompose) {
        ...
        for (String scenario : FrtbConstants.SCENARIOS) {
            ...
            // 总资本公式：Total Capital = sqrt(max(0, sum(Kb^2) + sum(rslt_bc)))
    ...
    private List<Map<String, Object>> calculateKL(List<Map<String, Object>> dataList) {
        ...
        for (Map<String, Object> k : dataList) {
            ...
            for (Map<String, Object> l : dataList1) {
                ...
                // CSRNS Curvature 监管口径只适用 name 相关性，不适用 tenor / basis 维度。
                boolean sameName = Objects.equals(k.get("riskFactorId"), l.get("riskFactorId"));
                double baseRho = sameName ? 1.0 : getNameDiffRhoByBucket(bucketK);

                for (String scenario : FrtbConstants.SCENARIOS) {
                    double curvatureRho = sameName
                            ? 1.0
                            : FrtbConstants.applyCurvatureScenarioStress(scenario, baseRho);
    ...
    private double lookupGamma(String bucketB, String bucketC) {
        ...
        if (gamma == null) {
            throw new IllegalArgumentException("未配置 CSRNS Curvature 跨桶相关性: " + bucketB + "," + bucketC);
        }
        return gamma;
    }
