...
public class CsrnsVega {
    ...
    private static final HashMap<String, Double> CSRNS_GAMMA_MATRIX = FrtbParamsCache.buildCsrnsGammaMatrix();
    ...
    public Map<String, Object> calculate(List<Map<String, Object>> dataList, Boolean needDecompose) {
        ...
        // 3. 计算最终资本
        Map<String, Object> csrv = new HashMap<>();
        ...
        for (String scenario : FrtbConstants.SCENARIOS) {
            ...
            csrv.put("capital_" + scenarioName, est);
        ...
        csrv.put("capital", finalCapital);
        csrv.put("riskFactorClass", RISK_CLASS);
        csrv.put("sensType", SENS_TYPE);
        csrv.put("capital_normal", capital_M);
        csrv.put("capital_high", capital_H);
        csrv.put("capital_low", capital_L);

        // 4. (可选) 资本分解
        List<Map<String, Object>> decompRsltList = new ArrayList<>();
        if (Boolean.TRUE.equals(needDecompose)) {
            decompRsltList = decompose(dataList, klList, aggList, bcList, csrv);
        }
    ...
    private List<Map<String, Object>> calculateKL(List<Map<String, Object>> dataList) {
        ...
        for (Map<String, Object> k : dataList) {
            ...
            for (Map<String, Object> l : dataList1) {
                ...
                // CSRNS Vega 仅完全同一风险因子视为自相关，其余使用基础 rho
                double baseRho = FrtbParamsCache.getCsrnsRhoBase();
                boolean isSelf = isSameRiskFactor(k, l);
                if (isSelf)
                    baseRho = 1.0;

                double currentBaseRho = baseRho;
    ...
    private List<Map<String, Object>> decompose(List<Map<String, Object>> dataList,
        ...
        for (Map<String, Object> trade : dataList) {
            ...
            for (String scenario : FrtbConstants.SCENARIOS) {
                ...
                double capital = getDouble(csrv, "capital_" + scenName);
    ...
    private double lookupGamma(String bucketB, String bucketC) {
        ...
        Double gamma = CSRNS_GAMMA_MATRIX.get(bucketB + "," + bucketC);
        if (gamma == null) {
            throw new IllegalArgumentException("未配置 CSRNS Vega 跨桶相关性: " + bucketB + "," + bucketC);
        }
        return gamma;
    }
