...
public class EqDelta {
    ...
    public Map<String, Object> calculate(List<Map<String, Object>> dataList, Boolean needDecompose) {
        ...
        // 3. 计算最终资本
        Map<String, Object> eqd = new HashMap<>();
        ...
        for (String scenario : FrtbConstants.SCENARIOS) {
            ...
            // EQ Delta 聚合：Capital = Sqrt(Sum_{b,c} term)
            ...
            eqd.put("capital_" + scenarioName, est);
        ...
        eqd.put("capital", finalCapital);
        eqd.put("riskFactorClass", RISK_CLASS);
        eqd.put("sensType", SENS_TYPE);
        eqd.put("capital_normal", capital_M);
        eqd.put("capital_high", capital_H);
        eqd.put("capital_low", capital_L);

        // 4. (可选) 资本分解
        List<Map<String, Object>> decompRsltList = new ArrayList<>();
        if (Boolean.TRUE.equals(needDecompose)) {
            decompRsltList = decompose(dataList, klList, aggList, bcList, eqd);
        }
        ...
        for (String bucket : allBuckets) {
            ...
            for (String scenario : FrtbConstants.SCENARIOS) {
                ...
                double Sb = dataList.stream()
                        ...
                        .mapToDouble(e -> getDouble(e, "ws")) // 加权敏感度总和
                        .sum();
        ...
        for (Map<String, Object> b : aggList) {
            for (Map<String, Object> c : aggList) {
                ...
                for (String scenario : FrtbConstants.SCENARIOS) {
                    ...
                    putByScenario(bc, "gammac", scenario, gammac); // 用于资本分解
                }
                bcList.add(bc);
            }
        }
    ...
    private List<Map<String, Object>> decompose(List<Map<String, Object>> dataList,
        ...
        for (Map<String, Object> trade : dataList) {
            ...
            for (String scenario : FrtbConstants.SCENARIOS) {
                ...
                double capital = getDouble(eqd, "capital_" + scenName);
