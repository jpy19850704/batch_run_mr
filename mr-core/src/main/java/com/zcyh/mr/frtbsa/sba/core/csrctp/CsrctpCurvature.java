...
/**
 * CSR CTP Curvature 资本计算
 * 信用利差风险 — 证券化（关联交易组合）Curvature 敏感性
 *
 * 根据 MAR21.56-21.64：
 * - Curvature 使用 CVR_up 和 CVR_down 的较大值
 * - 桶内相关性：Curvature 只适用 name 相关性，同一发行人 ρ = 1.0，不同发行人 ρ = rhoBase
 * - 跨桶相关性：γ² 用于 Curvature 聚合
 * - Psi 函数：当两个 CVR 均为负时取 0
 */
public class CsrctpCurvature {
    ...
    private static final HashMap<String, Double> CSRCTP_GAMMA_MATRIX = FrtbParamsCache.buildCtpDeltaGammaMatrix();
    ...
    private List<Map<String, Object>> calculateKL(List<Map<String, Object>> dataList) {
        ...
        for (Map<String, Object> k : dataList) {
            ...
            for (Map<String, Object> l : dataList1) {
                ...
                // CSRCTP Curvature 监管口径只适用 name 相关性，不适用 tenor / basis 维度。
                boolean sameIssuer = Objects.equals(k.get("riskFactorId"), l.get("riskFactorId"));
                double currentBaseRho = sameIssuer ? 1.0 : baseRho;

                for (String scenario : FrtbConstants.SCENARIOS) {
                    double curvatureRho = sameIssuer
                            ? 1.0
    ...
    private List<Map<String, Object>> decompose(List<Map<String, Object>> dataList,
        ...
        for (String scenario : FrtbConstants.SCENARIOS) {
            ...
            double capitalTotal = getDouble(csrc, "capital_" + scenName);
    ...
    private double lookupGamma(String bucketB, String bucketC) {
        Double gamma = CSRCTP_GAMMA_MATRIX.get(bucketB + "," + bucketC);
        if (gamma == null) {
            throw new IllegalArgumentException("未配置 CSRCTP Curvature 跨桶相关性: " + bucketB + "," + bucketC);
        }
        return gamma;
    }
