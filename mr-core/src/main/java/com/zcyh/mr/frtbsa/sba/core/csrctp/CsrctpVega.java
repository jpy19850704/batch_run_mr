...
/**
 * CSR CTP Vega 资本计算
 * 信用利差风险 — 证券化（关联交易组合）Vega 敏感性
 *
 * 根据 MAR21.56-21.64：
 * - Vega 风险权重 = 100%（与所有其他风险类别一致）
 * - 桶内相关性：完全同一风险因子 ρ = 1.0，其余使用 rhoBase (0.42)
 * - 跨桶相关性：γ = 0.18
 */
public class CsrctpVega {
    ...
    private List<Map<String, Object>> calculateKL(List<Map<String, Object>> dataList) {
        ...
        for (Map<String, Object> k : dataList) {
            ...
            for (Map<String, Object> l : dataList1) {
                ...
                double currentBaseRho = isSelf ? 1.0 : baseRho;

                for (String scenario : FrtbConstants.SCENARIOS) {
                    double rho = isSelf ? 1.0 : FrtbConstants.applyScenarioStress(scenario, currentBaseRho);
    ...
    private Map<String, List<Map<String, Object>>> calculateAgg(List<Map<String, Object>> klList,
        ...
        double baseGammaVal = FrtbParamsCache.getCtpGamma();
        ...
        for (Map<String, Object> b : aggList) {
            for (Map<String, Object> c : aggList) {
                ...
                double baseGamma = bucketB.equals(bucketC) ? 1.0 : baseGammaVal;
