...
/**
 * CSRNC Curvature 资本计算逻辑
 * 信用利差风险 (非证券化) - Curvature敏感性
 */
public class CsrncCurvature {
    ...
    public Map<String, Object> calculate(List<Map<String, Object>> dataList, Boolean needDecompose) {
        ...
        for (String scenario : FrtbConstants.SCENARIOS) {
            ...
            double est = Math.sqrt(Math.max(0.0, sumKbSq + sumRsltBc)) + kbSpecial;
    ...
    private List<Map<String, Object>> calculateKL(List<Map<String, Object>> dataList) {
        ...
        for (Map<String, Object> k : dataList) {
            ...
            for (Map<String, Object> l : dataList1) {
                ...
                boolean isSelf = Objects.equals(k.get("riskFactorId"), l.get("riskFactorId"))
                        && isSameVertex(tenorK, tenorL);
                double baseRho = isSelf ? 1.0 : FrtbParamsCache.getCsrncRhoBase();
    ...
    private double lookupGamma(String bucketB, String bucketC) {
        ...
        if (gamma == null) {
            throw new IllegalArgumentException("未配置 CSRNC Curvature 跨桶相关性: " + bucketB + "," + bucketC);
        }
        return gamma;
    }
